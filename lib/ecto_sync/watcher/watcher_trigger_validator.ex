defmodule EctoSync.Watcher.WatcherTriggerValidator do
  ...
  def run(_) do
    ...
    |> Enum.each(fn {repo_mod, {extra_found_triggers, extra_found_functions}} ->
      if System.get_env("ECTO_SYNC_CLEANUP") == "cleanup" do
  ...
  defp log_extra_triggers(extra_found_triggers) do
    Logger.error("""
    Found the following extra EctoSync.Watcher triggers:
    ...
    To cleanup unspecified triggers and functions, run your app with the `ECTO_SYNC_CLEANUP`
  ...
  defp log_extra_functions(extra_found_functions) do
    Logger.error("""
    Found the following extra EctoSync.Watcher functions:
